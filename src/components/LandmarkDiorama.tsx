...
import { mountDiorama } from '@/components/diorama/mountDiorama';
...
export default function LandmarkDiorama({
  ...
  useEffect(() => {
    ...
    const dispose = mountDiorama(canvas, scene.glbUrl, scene.kind, {
    ...
    >
      ...
      {status !== 'ready' && (
        <div
          ...
          style={{ background: `${scene.backdrop}b3` }}
        >
          <div className="pointer-events-auto rounded-[24px] border-[3px] border-white bg-[rgba(243,230,200,0.94)] px-6 py-5 text-center shadow-sticker">
            {status === 'loading' ? (
